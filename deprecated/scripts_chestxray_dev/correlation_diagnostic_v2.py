"""
Correlation diagnostic for chest X-ray BiomedCLIP concept scores — v2.

Mirror of correlation_diagnostic.py pointed at the v2 (18-concept) feature set.
The pre-registered hypotheses H1-H5 were registered on v1; v2 results here are
a FOLLOW-UP check after targeted prompt revision, not a fresh pre-registration.

normal_anchor_block drops no_focal_opacity (removed in v2) → 4 members.
consolidation_block and interstitial_block are unchanged from v1.

Operates on train pool only (test indices never touched).
v1 artefacts are read-only; this script writes only to the _v2 output dir.

Outputs:
    results/chestxray/correlation_diagnostic_v2/correlation_matrix.npy
    results/chestxray/correlation_diagnostic_v2/correlation_heatmap.png
    results/chestxray/correlation_diagnostic_v2/summary.txt

Run from project root:
    python scripts/chestxray/correlation_diagnostic_v2.py
"""
...
OUT_DIR     = _ROOT / "results/chestxray/correlation_diagnostic_v2"

# ── Pre-registered block structure (v2: no_focal_opacity dropped) ─────────────
# Same blocks as v1 except normal_anchor_block loses no_focal_opacity (4 members).
...
HYPOTHESES = {
    ...
    "H5": "Aggregate mean |r| over all 153 pairs falls in [0.3, 0.6], lower than HAM10000's 0.475",
}
...
def plot_heatmap(
    ...
    ax.set_title("Chest X-ray BiomedCLIP Concept Correlation Matrix — v2 (train pool)",
                 fontsize=11, pad=12)
    plt.tight_layout()
...
def build_summary(
    ...
    lines.append("=" * 70)
    lines.append("CHEST X-RAY CONCEPT CORRELATION DIAGNOSTIC — v2")
    lines.append("(follow-up after targeted prompt revision; H1-H5 pre-registered on v1)")
    lines.append("=" * 70)
    ...
    lines.append("=" * 70)
    lines.append("Pre-registered predictions (registered on v1; v2 = follow-up check):")
    lines.append("=" * 70)
