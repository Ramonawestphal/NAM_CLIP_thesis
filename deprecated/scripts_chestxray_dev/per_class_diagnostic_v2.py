"""
Per-class concept score diagnostic for chest X-ray BiomedCLIP v2 features.

Mirror of per_class_diagnostic.py pointed at v2 artefacts.
EXPECTED_TARGET mapping is identical to v1 (expected class does not change
with prompt wording — only discriminability should improve).

Operates on train pool only. Test indices are never touched.
v1 artefacts are read-only; this script does not modify them.

Outputs:
    results/chestxray/per_class_diagnostic_v2/per_class_means.csv
    results/chestxray/per_class_diagnostic_v2/per_class_heatmap.png
    results/chestxray/per_class_diagnostic_v2/discrimination_summary.txt
    results/chestxray/per_class_diagnostic_v2/h4_investigation.txt

Run from project root:
    python scripts/chestxray/per_class_diagnostic_v2.py
"""
...
# ── Paths ─────────────────────────────────────────────────────────────────────
SCORES_NPZ  = _ROOT / "data/features/biomedclip/chestxray_concept_scores_v2.npz"
OUTER_SPLIT = _ROOT / "data/splits/chestxray_outer_split.npz"
OUT_DIR     = _ROOT / "results/chestxray/per_class_diagnostic_v2"

# ── Block membership ──────────────────────────────────────────────────────────
# v2 has 18 concepts: 13 unchanged + 5 revised; no_focal_opacity DROPPED from v1.
# normal_anchor_block reverts to 4 members (no_focal_opacity removed).
NORMAL_ANCHOR_BLOCK = [
    "clear_lung_fields",
...
def plot_heatmaps(
    ...
    for ax, data, cmap, title, vrange, fmt in [
        (axes[0], means,   "viridis", "Absolute mean cosine similarity (v2)",
         (means.min(), means.max()), "{:.3f}"),
        (axes[1], z_means, "RdBu_r",  "Z-scored means per-concept across classes (v2)",
         (-2.5, 2.5), "{:.2f}"),
    ]:
    ...
    fig.suptitle("Chest X-ray BiomedCLIP v2: per-class concept scores (train pool)",
                 fontsize=10, y=1.01)
    plt.tight_layout()
...
def build_discrimination_summary(
    ...
    lines: List[str] = []
    lines.append("=" * 70)
    lines.append("PER-CONCEPT DISCRIMINATION SUMMARY v2 (train pool)")
    lines.append("=" * 70)
...
def build_h4_investigation(
    ...
    lines: List[str] = []
    lines.append("=" * 70)
    lines.append("H4 INVESTIGATION v2: normal_anchor_block × interstitial_block")
    lines.append("=" * 70)
    lines.append(
        "Repeating H4 investigation on v2 features to check whether prompt\n"
        "revision changed the between-block correlation pattern.\n"
        f"normal_anchor_block members (v2): {NORMAL_ANCHOR_BLOCK}\n"
        f"interstitial_block  members (v2): {INTERSTITIAL_BLOCK}\n"
    )
    ...
    for cname in NORMAL_ANCHOR_BLOCK:
        if cname not in concept_names:
            lines.append(f"  {cname:<40s}  NOT IN V2 CONCEPT SET")
            continue
