"""
Shape function inspection for NAM v6 Phase-1 (BiomedCLIP v6 features, 24 concepts).

Loads the best seed's checkpoint (highest test balanced accuracy) from
reports/nam/v6_base/ and visualises what the NAM learned per concept.
...
Outputs -> reports/nam/v6_base/shape_functions/

Run from project root after train_nam_v6.py has completed:
    python scripts/inspect_shape_functions_v6.py
"""
...
MODEL_DIR     = "reports/nam/v6_base"
OUT_DIR       = "reports/nam/v6_base/shape_functions"

# ── Architecture (must match train_nam_v6.py exactly) ────────────────────────
HIDDEN_DIMS = (32, 16)
DROPOUT     = 0.25
N_FEATURES  = 24
N_CLASSES   = 7
...
RUG_ALPHA   = 0.15    # opacity of rug marks (many overlap with nv-heavy data)
RUG_HEIGHT  = 0.04    # fraction of y-range for rug marks
RUG_SAMPLE  = 500     # max rug marks to draw (subsample large classes)
...
scores_raw  = feat["scores"]             # (10015, 24)
concept_ids = list(feat["concept_ids"])  # 24 strings

print("Loading splits...")
train_idx = np.load(SPLITS_PATH)["train_idx"]
X_train_raw = scores_raw[train_idx]     # (8020, 24) — for empirical ranges + rug

print("Loading scaler...")
with open(os.path.join(MODEL_DIR, "scaler.pkl"), "rb") as f:
    scaler = pickle.load(f)
X_train_sc = scaler.transform(X_train_raw).astype(np.float32)  # (8020, 24) z-scored
...
def compute_shape_fn(col: int) -> tuple[np.ndarray, np.ndarray]:
    """Grid [p1, p99] + shape function for concept `col`.

    Returns:
        grid (GRID_POINTS,)      — x values in z-score space
        fn   (GRID_POINTS, C)    — per-class log-odds contribution
    """
    lo, hi = np.percentile(X_train_sc[:, col], [1, 99])
    grid   = np.linspace(lo, hi, GRID_POINTS).astype(np.float32)
    fn     = model.concept_contributions(grid, col).numpy()   # (GRID_POINTS, C)
    return grid, fn


def range_score(fn: np.ndarray) -> float:
    """Sum over classes of (max - min) contribution across the grid."""
    return float(np.sum(np.abs(fn.max(axis=0) - fn.min(axis=0))))


def curvature_score(grid: np.ndarray, fn: np.ndarray) -> float:
    """L2 deviation from best-fit line, summed over classes."""
    total = 0.0
...
def add_rug(ax: plt.Axes, col_data: np.ndarray, y_min: float, y_max: float) -> None:
    """Draw a rug plot along the x-axis showing training-data density."""
...
# ─────────────────────────────────────────────────────────────────────────────
# Summary grid (4×6, all 24 concepts)
# ─────────────────────────────────────────────────────────────────────────────
...
for col_idx, cid in enumerate(concept_ids):
    ...
    # Lightweight rug on summary grid (fewer marks, smaller)
...
fig.suptitle(
    f"NAM v6 Phase-1 — Shape Functions (seed {best_seed}, z-scored inputs)",
    fontsize=11, y=1.01,
)
plt.tight_layout()
...
SUMMARY = f"""
==== Shape Function Analysis (NAM v6 Phase-1, seed {best_seed}) ====
