"""Prompt quality analysis for BiomedCLIP HAM10000 concept scores (v4 prompts).

Mirrors scripts/analyze_prompts.py but points at data/features/biomedclip/
ham10000_concept_scores_v4.npz and writes reports to
reports/prompt_analysis/biomedclip/. Uses the same train/test split as ViT-B/32.

Outputs (all under reports/prompt_analysis/biomedclip/):
...
Run from project root:
    python scripts/analyze_prompts_biomedclip.py
"""
...
# Paths
# ---------------------------------------------------------------------------
SCORES_NPZ = _ROOT / "data/features/biomedclip/ham10000_concept_scores_v4.npz"
SPLIT_NPZ  = _ROOT / "data/splits/train_test_lesion_split.npz"
REPORT_DIR = _ROOT / "reports/prompt_analysis/biomedclip"
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    if not SCORES_NPZ.exists():
        raise FileNotFoundError(
            f"BiomedCLIP scores not found: {SCORES_NPZ}\n"
    ...
    print("Prompts : v4 (ham10000_prompts_v4.txt, embedded in NPZ at extract time)")
    ...
    print(f"Loaded BiomedCLIP scores: {scores.shape}")
