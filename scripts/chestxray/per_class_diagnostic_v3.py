"""
Per-class concept score diagnostic for chest X-ray BiomedCLIP v3 features.

Mirror of per_class_diagnostic_v2.py pointed at v3 artefacts.
...
Outputs:
    results/chestxray/per_class_diagnostic_v3/per_class_means.csv
    results/chestxray/per_class_diagnostic_v3/per_class_heatmap.png
    results/chestxray/per_class_diagnostic_v3/discrimination_summary.txt
    results/chestxray/per_class_diagnostic_v3/h4_investigation.txt

Run from project root:
    python scripts/chestxray/per_class_diagnostic_v3.py
"""
...
# ── Paths ─────────────────────────────────────────────────────────────────────
SCORES_NPZ  = _ROOT / "data/features/biomedclip/chestxray_concept_scores_v3.npz"
OUTER_SPLIT = _ROOT / "data/splits/chestxray_outer_split.npz"
OUT_DIR     = _ROOT / "results/chestxray/per_class_diagnostic_v3"

# ── Block membership ──────────────────────────────────────────────────────────
# v3 has 18 concepts: 16 frozen from v2 + 2 smoke-test winners. Block membership
...
def plot_heatmaps(
    ...
    for ax, data, cmap, title, vrange, fmt in [
        (axes[0], means,   "viridis", "Absolute mean cosine similarity (v3)",
         (means.min(), means.max()), "{:.3f}"),
        (axes[1], z_means, "RdBu_r",  "Z-scored means per-concept across classes (v3)",
         (-2.5, 2.5), "{:.2f}"),
    ]:
    ...
    fig.suptitle("Chest X-ray BiomedCLIP v3: per-class concept scores (train pool)",
                 fontsize=10, y=1.01)
    plt.tight_layout()
...
def build_discrimination_summary(
    ...
    lines: List[str] = []
    lines.append("=" * 70)
    lines.append("PER-CONCEPT DISCRIMINATION SUMMARY v3 (train pool)")
    lines.append("=" * 70)
...
def build_h4_investigation(
    ...
    lines: List[str] = []
    lines.append("=" * 70)
    lines.append("H4 INVESTIGATION v3: normal_anchor_block × interstitial_block")
    lines.append("=" * 70)
    lines.append(
        "Repeating H4 investigation on v3 features to check whether prompt\n"
        "revision changed the between-block correlation pattern.\n"
        f"normal_anchor_block members (v3): {NORMAL_ANCHOR_BLOCK}\n"
        f"interstitial_block  members (v3): {INTERSTITIAL_BLOCK}\n"
    )
    ...
    for cname in NORMAL_ANCHOR_BLOCK:
        if cname not in concept_names:
            lines.append(f"  {cname:<40s}  NOT IN V3 CONCEPT SET")
            continue
