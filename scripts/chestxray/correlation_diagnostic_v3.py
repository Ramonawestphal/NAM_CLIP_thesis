"""
Correlation diagnostic for chest X-ray BiomedCLIP concept scores — v3.

Mirror of correlation_diagnostic_v2.py pointed at the v3 (18-concept) feature
set. The pre-registered hypotheses H1-H5 were registered on v1; v3 results here
are a FOLLOW-UP check after the smoke-test viral-tier revision, not a fresh
pre-registration.

BLOCKS are unchanged from v2: normal_anchor_block has 4 members;
consolidation_block and interstitial_block are unchanged.

Operates on train pool only (test indices never touched).
v1/v2 artefacts are read-only; this script writes only to the _v3 output dir.

Outputs:
    results/chestxray/correlation_diagnostic_v3/correlation_matrix.npy
    results/chestxray/correlation_diagnostic_v3/correlation_heatmap.png
    results/chestxray/correlation_diagnostic_v3/summary.txt

Run from project root:
    python scripts/chestxray/correlation_diagnostic_v3.py
"""
...
OUT_DIR     = _ROOT / "results/chestxray/correlation_diagnostic_v3"
...
def plot_heatmap(
    ...
    ax.set_title("Chest X-ray BiomedCLIP Concept Correlation Matrix — v3 (train pool)",
                 fontsize=11, pad=12)
    plt.tight_layout()
...
def build_summary(
    ...
    lines.append("=" * 70)
    lines.append("CHEST X-RAY CONCEPT CORRELATION DIAGNOSTIC — v3")
    lines.append("(follow-up after smoke-test viral-tier revision; H1-H5 pre-registered on v1)")
    lines.append("=" * 70)
    ...
    lines.append("=" * 70)
    lines.append("Pre-registered predictions (registered on v1; v3 = follow-up check):")
    lines.append("=" * 70)
