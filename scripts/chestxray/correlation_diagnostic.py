"""
Correlation diagnostic for chest X-ray BiomedCLIP concept scores.

Analogue of the HAM10000 input correlation analysis (mean |r| = 0.475 on
24-dim space; same procedure on the 19-dim space here).

Operates on train pool only (test indices never touched).

Outputs:
    results/chestxray/correlation_diagnostic/correlation_matrix.npy
    results/chestxray/correlation_diagnostic/correlation_heatmap.png
    results/chestxray/correlation_diagnostic/summary.txt

Run from project root:
    python scripts/chestxray/correlation_diagnostic.py
"""
...
OUT_DIR     = _ROOT / "results/chestxray/correlation_diagnostic"

# ── Pre-registered block structure ────────────────────────────────────────────
# Matches the pre-registered blocks from the task spec. The 19-concept v1 prompt
# file includes no_focal_opacity, which is part of normal_anchor_block.
BLOCKS = {
    "consolidation_block": [
        "consolidation",
        "focal_opacity",
...
HYPOTHESES = {
    ...
    "H5": "Aggregate mean |r| over all 171 pairs falls in [0.3, 0.6], lower than HAM10000's 0.475",
}
...
def plot_heatmap(
    ...
    ax.set_title("Chest X-ray BiomedCLIP Concept Correlation Matrix (train pool)",
                 fontsize=11, pad=12)
    plt.tight_layout()
...
def build_summary(
    ...
    lines.append("=" * 70)
    lines.append("CHEST X-RAY CONCEPT CORRELATION DIAGNOSTIC")
    lines.append("=" * 70)
    ...
    lines.append("=" * 70)
    lines.append("Pre-registered predictions (recorded before observing results):")
    lines.append("=" * 70)
